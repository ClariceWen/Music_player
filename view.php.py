...
class test_complexwave(unittest.TestCase):
    ...
    def test_get_duration(self):
        ...
        for wave in wave_list:
            temp += wave
        self.assertTrue(temp.get_duration() == 999 or temp.get_duration() == 1)
    ...
    def test_get_duration_2(self):
        ...
        for wave in wave_list:
            temp += wave
        self.assertTrue(temp.get_duration() == max(durations) or temp.get_duration() == 1)

    def test_play(self):
        exp = self.complex2.play()
        act = (self.simple + self.simple2).play()
        self.assertTrue(len(exp) == helper._SAMPLE_RATE or len(exp) == 0.5 * helper._SAMPLE_RATE)
...
class test_sawtooth(unittest.TestCase):
    ...
    def test_setUp(self):
        self.assertTrue(len(SawtoothWave.mro()) >= 3)
    ...
    def test_max(self):
        self.assertTrue(max(self.saw.play()) <= 1)
        self.assertTrue(min(self.saw.play()) >= -1)

    def test_max2(self):
        temp = SawtoothWave(440, 0.5, 0.5)
        self.assertTrue(max(temp.play()) <= 1)
        self.assertTrue(min(temp.play()) >= -1)
...
class test_square(unittest.TestCase):
    ...
    def test_setUp(self):
        self.assertTrue(len(SquareWave.mro())>=3)
    ...
    def test_max(self):
        self.assertTrue(max(self.squ.play()) <= 1)
        self.assertTrue(min(self.squ.play()) >= -1)

    def test_max2(self):
        temp = SquareWave(440, 0.5, 0.5)
        self.assertTrue(max(temp.play()) <= 1)
        self.assertTrue(min(temp.play()) >= -1)
...
class test_rest(unittest.TestCase):
    def setUp(self):
        self.rest = Rest(10)
        self.assertTrue(len(Rest.mro()) >=3 )
...
class test_stutternote(unittest.TestCase):
    ...
    def test_init_corner2(self):
        temp = StutterNote(440, 1.026, 1).get_waves()
        self.assertEqual(42, len(temp), "You should add an extra wave beyond 40 simple waves for the extra 0.01 second")
        (num_swa, num_rset) = count_wave(temp)
        self.assertEqual(True, num_swa >= 21, "You should have at least 20 SawtoothWave")
        self.assertEqual(True, num_rset >= 21, "You should have at least 20 Rset")
        self.assertEqual(True, num_rset + num_swa == 42,
                         "You should only contain SawtoothWave and Rset in your solution")
...
class test_Holophonor(unittest.TestCase):
    ...
    def test_getduration2(self):
        ...
        try:
            ...
            print("This one is an optional test, this case will not be tested definitly you can do your best to handle it")
...
if __name__ == "__main__":
    ...
    baliset = Baliset()
    note_info = [("1:2", 1, 1)]
    note_info3 = [("1:2", 0.5, 0.5), ("1:4", 0.5, 0.5)]
    baliset.next_notes(note_info)
    plot_wave(baliset.play())
    baliset.next_notes(note_info3)
    plot_wave(baliset.play())
    holophonor = Holophonor()
    note_info = [("1:5", 1, 1)]
    note_info3 = [("1:5", 0.5, 0.5), ("1:13", 0.5, 0.5)]
    holophonor.next_notes(note_info)
    plot_wave(holophonor.play())
    holophonor.next_notes(note_info3)
    plot_wave(holophonor.play())
    gaff = Gaffophone()
    note_info = [("2:1", 1, 1)]
    note_info3 = [("2:1", 0.5, 0.41), ("4:1", 0.5, 0.59)]
    gaff.next_notes(note_info)
    plot_wave(gaff.play())
    gaff.next_notes(note_info3)
    plot_wave(gaff.play())
